...
1. 从 [最新 Release](https://github.com/orionhu99/fMOST-Brain-Viewer/releases/latest)
   下载 `fMOST-Brain-Viewer-Setup-2.3.0-win64.exe`。
...
- 按 acronym、全名或 Allen structure ID 高亮任意脑区；通用示例为 `MOp`、`VISp`。
...
软件不包含遥测，不会上传用户数据。只有用户主动选择下载 Allen 图谱或 ontology 时，
软件才会访问网络。SWC、CSV 和 NRRD 输入始终只读；派生缓存、设置、日志、截图和
...
- 软件内：**Help > Open log folder** 和 **Help > About fMOST Brain Viewer**
...
当前版本：**2.3.0 — Initial public release**。
